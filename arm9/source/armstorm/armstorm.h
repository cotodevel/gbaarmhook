...
enum EndianityType {ENDIANITY_BIG, ENDIANITY_LITTLE};
typedef unsigned long long AddressType;
enum OperandType {OPERAND_NONE, OPERAND_REG, OPERAND_IMM, OPERAND_ADDR, OPERAND_REGLIST};
enum DecodeResult{DECRES_SUCCESS, DECRES_INPUTERR};
...
struct DInst{
    /* __OUT */ AddressType   address;     /* Address of current instruction. */
    /* __OUT */ AddressType   target;      /* Target address of a branch instruction. */
    /* __OUT */ unsigned short opcode;      /* The opcode id in _ThumbOpcodeType. */
    /* __OUT */ iflags        flags;       /* See flags above. */
    /* __OUT */ struct Operand       operands[3]; /* Up to three operands of struct _Operand. */
};

struct DecomposeInfo{
    /* __IN */ AddressType         address;         /* An ODD ADDRESS will cause to decode THUMB instructions! */
    ...
    /* __IN */ enum EndianityType       endianity;       /* Decode instructions in big/little endian. */
    /* __IN */ unsigned int         maxInstructions; /* Number of instructions that can be written to 'instructions'. */

    /* __OUT */ struct DInst*      instructions ;            /* Decoded instructions are written to 'instructions'. */
...
extern struct DInst insts[];
extern struct DecomposeInfo info;
extern struct TInst tinst;
extern struct Operand operand;
extern struct TInst tinst;
...
extern void armstorm_format(const struct DecomposeInfo* ci, struct DInst* inst, struct TInst* text);
extern enum DecodeResult armstorm_decompose(struct DecomposeInfo* info);
