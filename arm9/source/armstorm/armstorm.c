...
enum DecodeResult armstorm_decompose(struct DecomposeInfo* info)
{
...
static void format_operand(char* str, const struct Operand* op, const struct DInst* pdi)
{
    char buf[128];
    switch (op->type)
    {
...
void armstorm_format(const struct DecomposeInfo* ci, struct DInst* inst, struct TInst* text)
{
