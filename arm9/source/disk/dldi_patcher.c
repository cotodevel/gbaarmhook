...
const u32 _io_dldi; //ori: extern const u32 _io_dldi; (not added to dldi_patcher.h)
...
int runNdsFile (const char* filename, int argc, const char** argv)  {
	struct stat st;
	char filePath[MAX_LFN_LENGTH * 2];
	int pathLen;
	const char* args[1];
	...
	if (argc <= 0 || !argv) {
		...
		if (!getcwd (filePath, MAX_LFN_LENGTH)) {
			return 2;
		}
	...
	return runNds (filename, 0, st.st_ino, true, true, argc, argv);
}