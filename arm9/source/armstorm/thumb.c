...
static const struct ThumbInstInfo* decompose_thumb_lookup_instruction(unsigned short instruction, unsigned short nextInstruction)
{
    ...
    if (indexer == 0xf000) {
        ...
        if (indexer == 0xe800) { /* BLX suffix. */
            ...
            static struct ThumbInstInfo ThumbInfo_BLX = {I_BLX, FLAG_BIG_INST, OT_OFF11, OT_NONE, OT_NONE};
            return &ThumbInfo_BLX;
        }
        if (indexer == 0xf800) { /* BL suffix. */
            static struct ThumbInstInfo ThumbInfo_BL = {I_BL, FLAG_BIG_INST, OT_OFF11, OT_NONE, OT_NONE};
            return &ThumbInfo_BL;
        }
    }
                                   ...
                                   struct DInst* pdi)
{
    ...
    struct Operand* op = &pdi->operands[*opIndex];
    (*opIndex) += 1;
...
static int decompose_thumb_instruction(unsigned short instruction, unsigned short nextInstruction, struct DInst* pdi)
{
    int ret;
    unsigned int operandIndex = 0;
    const struct ThumbInstInfo* ii;
    iflags instFlags;
...
enum DecodeResult decompose_thumb(struct DecomposeInfo* info)
{
    unsigned int size;
    ...
    while ((info->codeLength > 0) && (instsNo < info->maxInstructions)) {
        ...
        memset((void*)pdi, 0, sizeof(struct DInst));
