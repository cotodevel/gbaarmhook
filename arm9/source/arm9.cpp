...
//vector creation
using namespace std;
vector<string> argarray; //dirnames (char)
vector<int> argarray2;//dir_properties (int)
vector<string> extensionList;//dirext allowed (char)
...
typedef struct{
	...
	u8 homebrew;
	u64 crc;
	char patchpath[MAX_LFN_LENGTH * 2];
	u8 swaplcd;
	u8 savfetype;
	u8 frameskip;
	u8 frameskipauto;
...
//iprintf("initemu(); / cpureset(); /bios_regramrst()! \n");
...
if(isgbaopen(gbaromfile)==0)
	iprintf("ready to open gbarom. ");
if (opengbarom((const char*)"/gba/rs-pzs.gba","r+")==0){
	iprintf("GBAROM open OK!");
	iprintf("GBAROM size is (%d) bytes", (int)getfilesizegbarom());
	}
else {
	iprintf("GBAROM open ERROR. close the nds");
	while(1);
}

//int gbaofset=0;
//iprintf("gbaromread @ %x:[%x]",(unsigned int)(0x08000000+gbaofset),(unsigned int)readu32gbarom(gbaofset));
...
//label asm patcher
	...
	iprintf("\n PATCH_BOOTCODE[0]: (%x) \n",(unsigned int)(PATCH_BOOTCODE_PTR[0]));
	iprintf("PATCH_START[0]: (%x) \n",(unsigned int)(PATCH_START_PTR[0]));
	iprintf("PATCH_HOOK_START[0]: (%x) \n",(unsigned int)(PATCH_HOOK_START_PTR[0]));
	...
	iprintf("\n >PATCH_BOOTCODE EXTRACT'd (%d) opcodes \n",(int)PATCH_BOOTCODE_SIZE);
	...
	iprintf("\n >PATCH_START EXTRACT'd (%d) ARM opcodes \n",(int)PATCH_START_SIZE);
	...
	iprintf("\n >PATCH_HOOK_START EXTRACT'd (%d) ARM opcodes \n",(int)PATCH_HOOK_START_SIZE);
	...
	iprintf("\n >NDS7_RTC_PROCESS EXTRACT'd (%d) ARM opcodes \n",(int)NDS7_RTC_PROCESS_SIZE);
	
	
	//ok
	//int i=0;
	//for(i=0;i<PATCH_HOOK_START_SIZE;i++){
		//iprintf("buf_wram(%d):[%x]",(int)i,(unsigned int)buf_wram[i+PATCH_BOOTCODE_SIZE+PATCH_START_PTR_SIZE]);
	//}
...
//so far code is stable, this will cause lockups definitely
	...
	irqSet(IRQ_VBLANK, vblank_thread);
	
	//irqSet(IRQ_HBLANK,hblank_thread);
	//irqSet(IRQ_VCOUNT,vcount_thread);
	//irqSet(IRQ_FIFO_NOT_EMPTY, fifo_thread);

	//upon data transfer IRQ_CARD
	irqEnable(IRQ_VBLANK/*|IRQ_HBLANK|IRQ_VCOUNT|IRQ_FIFO_NOT_EMPTY*/);
